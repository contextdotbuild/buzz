...
buzz messages send --channel <uuid> --content "Reply" --reply-to <event-id>              # reply inside the thread (threads are one level deep)
buzz messages send --channel <uuid> --content "Reply" --reply-to <event-id> --broadcast  # also show the reply on the channel timeline
