...
buzz-acp --agents 2 --heartbeat-interval 300 \
  ...
  --heartbeat-prompt "Run buzz schedules claim-due --limit 1 --lease-seconds 300. Read the claimed thread and named evidence. End schema-1 work with exactly one bind, complete, or reschedule transition; no newly formatted delegation is required. End schema-2 work with exactly one buzz schedules reconcile keep, wake, redirect, or complete decision. If none are due, publish nothing."
```
