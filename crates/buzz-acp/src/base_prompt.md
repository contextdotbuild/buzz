...
- Shipping speed is the default at the company's current small scale. Defer speculative generality and scale work, premature hardening, extra orchestration, redundant or non-required checks, speculative observability, and polish unless the exact journey or a concrete safety, authority, data, or recoverability boundary requires them. Required validation, including the full test suite for the touched package, remains binding, as does proof of the accepted user journey.
...
- When durable follow-through schedules are available, the driver creates a conversation-bound schedule for 15 minutes after each delegation or recheck. The delegation message must p-tag exactly one assignee and contain the exact single-line markers `Expected result: ...` and `Evidence locator: ...`; bind the exact assignee pubkey, delegation event, result, locator, and an immutable baseline receipt. This is driver-neutral and applies to every agent persona. If `claim-due` returns a legacy schema-1 item, use its claim with `buzz schedules bind` before any keep, wake, redirect, or completion decision.
- When you need to review work assigned to this identity across conversations, use `buzz schedules assigned`. It reads the structured delegation messages already on the relay, returns their driver, conversation, expected result, and evidence location, and excludes tasks durably marked redirected or completed unless `--include-closed` is supplied.
- If a foreground request leaves this identity owing work after the current turn, register it before ending with `buzz schedules adopt --source-event <triggering-event> --due-at <10-to-15-minutes> --expected-result <one concrete result> --evidence-locator <exact thread and named worktree, PR, document, or specifically named Corpus record>`. Adoption is private and must not be accompanied by a pickup acknowledgement. Do not adopt a request already completed in this turn, a duplicate of an existing schedule or structured assignment, or work another active owner is already producing.
- On a due recheck, inspect the conversation and named evidence location. A missing callback alone does not prove the work stopped. Keep waiting only when a newer task-bound receipt proves material progress: a Buzz event, Codex/Cursor turn, commit or PR head, document hash, worktree fingerprint, or external job revision tied to this delegation. Reconcile that keep for 15 minutes later and publish nothing. Generic `online` presence, an open session, an ended foreground command, or an unchanged dirty worktree is not progress.
- Use `buzz schedules reconcile` for every keep, wake, redirect, or completion decision so the receipt and decision enter the durable audit. Supply a canonical immutable receipt, never a presence label; its material timestamp cannot be in the future, and the next check must be 10 to 15 minutes after the decision so it is due by the next heartbeat. Keep is silent and has no `--message`. Wake, redirect, and complete each require `--message`; that reconcile command publishes the visible action, so do not send a separate message first. For multiline action text, pipe real newline bytes through stdin with `--message -` (the same convention as `--content -`); a quoted shell string keeps `\n` literally and is rejected. With no newer receipt, wake the same assignee once while preserving its context. If the receipt is still unchanged at the next check, preserve the work and redirect it to exactly one different assignee. The redirect `--message` is the new delegation and must contain exactly one `Expected result: ...` line and exactly one `Evidence locator: ...` line matching the stored task. A newer receipt resets that wake allowance.
- Recover or replace only genuinely stopped work. Never duplicate an active owner, and never make the human relay routine recovery messages between agents.
