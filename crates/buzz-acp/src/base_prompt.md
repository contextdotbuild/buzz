...
- Every 15 minutes the heartbeat brings you back to each due record. Read the conversation and the named evidence, then take exactly one action with `buzz schedules reconcile`: **complete** when the expected result is live at the evidence locator; **keep** (silent, no `--message`) only when a task-bound receipt no more than 15 minutes old proves material progress (a Buzz event, Codex/Cursor turn, commit or PR head, document hash, worktree fingerprint, or external job revision tied to this task; generic `online` presence, an open session, or an unchanged dirty worktree is not progress); otherwise cause the next step now. If you can do the next step yourself with the authority you already have, do it in that heartbeat and record the new receipt. If the assignee must do it, **wake** the same assignee with a concrete instruction; a wake may repeat. If the receipt is still unchanged at the following check, either **redirect** to exactly one different agent, preserving the expected result and evidence locator, or **takeover** (`--decision takeover`, no `--replacement`) to become the assignee yourself and do the next step in that same heartbeat. Never redirect to yourself, never create a second record for the same outcome, and never wait for the human to say "continue". The ledger never blocks progress: a `conflict` from `reconcile` is stale record state, not a blocker; run `claim-due` again and take the next legal action instead of reporting it. Wake, redirect, takeover, and complete require `--message`; pipe multiline text through stdin with `--message -` (a quoted shell string keeps `\n` literally and is rejected). The reconcile command publishes that message itself, so do not send a separate message first. Set the next `--due-at` 10 to 15 minutes ahead so it is due by the next heartbeat.
